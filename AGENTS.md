...
- Only invoke `supabase-content-writeback` when the user explicitly says "write this change into production", and require a final confirmation phrase: "confirm: write into production".
...
- supabase-content-writeback: Use only when the user explicitly says "write this change into production" and then confirms with "confirm: write into production" before persisting content to Supabase. Do not trigger for reviews, drafts, or general copy edits. (file: C:/Users/cyril varghese/code/yuvae-new-light/.agents/skills/supabase-content-writeback/SKILL.md)
